...
class eegModule:
    ...
    def visualize(self, i, new_sample):
        ...
        if i < 64: # add eeg data to array
            self.eeg_data[i] = new_sample
        else: # start plotting
            #store in temp so we can get the updated data and max for later use
            temp = self.getFreqBandOrValue(self.eeg_data, new_sample, -3, self.global_max)

            # get alpha band
            alpha_band = temp[0]
            # get total band
            total_band = self.getFreqBandOrValue(self.eeg_data, new_sample, 0, self.global_max)[0]
